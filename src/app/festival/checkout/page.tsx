...
export default function FestivalCheckoutPage() {
  ...
  // Dynamic capacity tracking based on selected ticket/promo
  const kapasitas = useMemo(() => {
    if (appliedPromo?.kapasitas) {
  ...
  // 1. Selected Option Availability (strictly from activePromos)
  ...
  // 2. Are all selectable options sold out?
  ...
  // Step 1 Validation Check (Participant Data, Ticket/Bundle Choice, Rules Agreement)
    ...
    const pollQuota = async () => {
      ...
      try {
        ...
        const [serverGuard, quotaCheck] = await Promise.all([
          validatePreCheckoutGuard("festival", registrantCount, appliedPromo?.id),
          checkQuotaAvailability("festival", registrantCount, appliedPromo?.id),
        ]);
  ...
  const handleAdvanceToStep2 = async (e?: React.MouseEvent) => {
    ...
    if (!persetujuanAturan) {
      const msg = "Anda harus menyetujui tata tertib Festival VOITSFEST 2026.";
      setError(msg);
      showToast("error", msg);
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const primaryRow: Record<string, any> = {
        ...
        group_id: groupId,
        is_primary: true,
        nama_lengkap: userData.fullName.trim() || authUser.user_metadata?.full_name || "",
        whatsapp: userData.phone.trim() || authUser.user_metadata?.phone || "",
        email: userData.email.trim() || authUser.email || "",
      ...
      if (regError && (regError.message?.includes("promo_proof_url") || regError.code === "PGRST204")) {
        console.warn("Retrying festival_registrations bulk insert without promo_proof_url:", regError.message);
        const fallbackArray = participantsArray.map((row) => {
          ...
          <form onSubmit={handleSubmit} className="space-y-10 max-w-4xl mx-auto">
            ...
            <div className="bg-slate-950/60 backdrop-blur-xl rounded-2xl p-6 md:p-10 relative overflow-hidden shadow-[0_4px_25px_rgba(0,0,0,0.5)] border border-white/10">
              <div className="mb-6 pb-4 border-b border-white/10">
                ...
                />
                <p className="text-xs text-slate-300 mt-1">
                  Pilih tiket reguler aktif atau paket promo bundling yang tersedia untuk Festival Musik &amp; Pameran Seni VOITSFEST 2026
                </p>
              </div>
                ...
                {loadingPromos ? (
                  ...
                  <div className="p-8 rounded-2xl bg-black/30 border border-white/10 text-center space-y-3">
                    ...
                    <p className="text-xs text-slate-400 max-w-md mx-auto">
                      Seluruh paket pendaftaran Festival disalurkan secara eksklusif melalui katalog promo &amp; bundling. Silakan hubungi panitia untuk informasi ketersediaan penawaran berikutnya.
                    </p>
                  </div>
                ) : (
              ...
              {/* Tata Tertib / Legal Consent Checkbox */}
                ...
                {!selectedPricingId ? (
                  ...
                  <p className="text-xs text-on-surface-variant/70 font-poppins">
                    {appliedPromo?.requires_proof_file && !promoProofFile
                      ...
                      ? "Harap setujui persyaratan khusus paket promo dan tata tertib di atas untuk melanjutkan ke pembayaran."
                      : !persetujuanAturan
                      ? "Harap centang persetujuan tata tertib Festival sebelum melanjutkan ke pembayaran."
                      : "Lengkapi seluruh data peserta, pilih paket aktif, dan setujui tata tertib di atas untuk melanjutkan ke pembayaran."}
                  </p>
                ) : null}
              </div>
            </div>
